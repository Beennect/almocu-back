...
describe("ProductService", () => {
    ...
    describe("getOne", () => {
        it("should return product from cache if available", async () => {
            const mockProduct = { _id: VALID_ID, name: "Pizza", brand: "Sadia", restaurantId: "res1" };
            (redis.get as jest.Mock).mockResolvedValue(JSON.stringify(mockProduct));

            const result = await service.getOne(VALID_ID, "res1");

            expect(redis.get).toHaveBeenCalledWith(`product:${VALID_ID}`);
        ...
        it("should return product from DB and set cache if not in cache", async () => {
            ...
            expect(ProductModel.findOne).toHaveBeenCalledWith({ _id: VALID_ID, restaurantId: "res1" });
            expect(redis.set).toHaveBeenCalled();
            expect(result).toEqual(mockProduct);
        });

        it("should return null if ID is invalid", async () => {
            const result = await service.getOne("invalid-id", "res1");
    ...
    describe("createOne", () => {
        it("should create product successfully", async () => {
            const mockProduct = { _id: VALID_ID, name: "Pizza", brand: "Sadia", price: 10, restaurantId: "res1" };
            const saveMock = jest.fn().mockResolvedValue(mockProduct);
            (ProductModel as any).mockImplementation(() => ({ save: saveMock }));

            const result = await service.createOne("Pizza", "Sadia", 10, "Massa com queijo", "res1");

            expect(result).toEqual(mockProduct);
            expect(redis.del).toHaveBeenCalledWith("products:res1");
        });
        ...
        it("should throw error when duplicate product (same name + brand + restaurantId)", async () => {
            ...
            await expect(service.createOne("Pizza", "Sadia", 10, "Massa com queijo", "res1"))
                .rejects.toThrow("Já existe um produto com esse nome e marca neste restaurante!");
        });
    });
    ...
    describe("createMany", () => {
        it("should throw error when duplicate products exist", async () => {
            ...
            const products = [
                { name: "Pizza", brand: "Sadia", price: 10, description: "Desc", restaurantId: "res1" },
                { name: "Pizza", brand: "Sadia", price: 12, description: "Desc2", restaurantId: "res1" },
            ];

            await expect(service.createMany(products))
                .rejects.toThrow("Um ou mais produtos já existem com o mesmo nome e marca neste restaurante!");
        });
    });

    describe("updateOne", () => {
        it("should update product and invalidate cache", async () => {
            const mockProduct = { _id: VALID_ID, name: "Pizza Updated", brand: "Sadia", restaurantId: "res1" };
            (ProductModel.findOneAndUpdate as jest.Mock).mockResolvedValue(mockProduct);

            const result = await service.updateOne(VALID_ID, "res1", { name: "Pizza Updated" });

            expect(ProductModel.findOneAndUpdate).toHaveBeenCalled();
            expect(redis.del).toHaveBeenCalledWith(`product:${VALID_ID}`);
            expect(redis.del).toHaveBeenCalledWith("products:res1");
            expect(result).toEqual(mockProduct);
        });
        ...
        it("should throw error if product not found", async () => {
            ...
            await expect(service.updateOne(VALID_ID, "res1", { name: "Pizza" }))
                .rejects.toThrow("Produto não encontrado!");
        });

        it("should throw error if ID is invalid", async () => {
            await expect(service.updateOne("invalid-id", "res1", { name: "Pizza" }))
        ...
        it("should throw error when update causes duplicate", async () => {
            ...
            await expect(service.updateOne(VALID_ID, "res1", { name: "Pizza", brand: "Sadia" }))
                .rejects.toThrow("Já existe um produto com esse nome e marca neste restaurante!");
        });
    });
    ...
    describe("deleteOne", () => {
        it("should delete product and invalidate cache", async () => {
            ...
            const result = await service.deleteOne(VALID_ID, "res1");
        ...
        it("should return false if ID is invalid", async () => {
            const result = await service.deleteOne("invalid-id", "res1");
            expect(result).toBe(false);
        });
    });
});
