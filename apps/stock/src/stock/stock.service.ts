...
export class StockService {
  ...
  async findOne(id: string, restaurantId: string): Promise<Stock> {
    ...
    if (!item) {
      throw new NotFoundException(
        'Item de estoque não encontrado nesta filial.',
      );
    }
    return item;
  }
  ...
  async update(
    ...
    if (!updated) {
      throw new NotFoundException(
        'Item de estoque não encontrado para atualização.',
      );
    }
  ...
  async adjustQuantity(
    ...
    if (!updated) {
      ...
      throw new BadRequestException(
        'A quantidade resultante não pode ser negativa.',
      );
    }
  ...
  async remove(id: string, restaurantId: string): Promise<void> {
    ...
    if (result.deletedCount === 0) {
      throw new NotFoundException(
        'Item de estoque não encontrado para remoção.',
      );
    }
  }
}
